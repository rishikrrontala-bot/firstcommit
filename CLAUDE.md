# CLAUDE.md: Rishik Rontala's entry for Beginner's Paradise – FirstCommit
...
**Deadline: Wed Sep 30, 2026 · 5:00 PM EDT.** Event page: https://firstcommit.devpost.com/
...
- **GitHub Pages deploys automatically from whatever branch you push** (`.github/workflows/pages.yml` builds `dist/`, `build/`, `out/` or `site/`, or a root `index.html`). Live URL: **https://rishikrrontala-bot.github.io/firstcommit/**. After each deploy, check the Actions run succeeded and load the URL headlessly.
- Finish on `main` (judges read the default branch). Try `git push origin HEAD:main`. If that's refused, open a PR and merge it with the REST API (`gh api -X POST repos/rishikrrontala-bot/firstcommit/pulls -f title=… -f head=<branch> -f base=main`, then `gh api -X PUT repos/rishikrrontala-bot/firstcommit/pulls/<n>/merge -f merge_method=squash`). If that's refused too, make "merge the PR" step 1 of HANDOFF.md.
